...
tg_sendText "Lunching (lunch r5x-userdebug)"
# Normal build steps
. build/envsetup.sh
lunch voltageos_r5x-userdebug
...
export BUILD_USERNAME=MangroveBlock
export BUILD_HOSTNAME=Mangrove-server
export KBUILD_BUILD_NAME=MangroveBlock
export KBUILD_BUILD_HOST=Mangrove-server
...
tg_sendText "Starting Compilation (mka r5x)"

make clean
mka r5x -j10 | tee build.txt

tg_sendText "Build completed! Uploading rom to gdrive"
rclone copy out/target/product/r5x/*UNOFFICIAL* suzu:segawa-builds -P || rclone copy out/target/product/r5x/*r5x*.zip suzu:segawa-builds -P || rclone copy out/target/product/r5x/*.zip suzu:segawa-builds -P
